...
int main(int argc, char * argv[]){
	if(argc != 2){
		printf("Uso: %s <tamanio del vector>\n", argv[0]);
		exit(1);
	}
    ...
    double tiempo_concurrente;
    
    int nhilos[5] = {1,2,4,8,16};
    for (int i = 0; i < 5; i++)
    {
        double sumaTiempo = 0;
        for (int j = 0; j < 5; j++)
            {
                int num_hilos = nhilos[i];
                int tamanioBloque = N / num_hilos; //tenemos que cada bloque es del tamaño correspondiente a la proporcion que le toca a cada hilo
            ...
            tiempo_concurrente = (end.tv_sec - start.tv_sec) + (end.tv_usec - start.tv_usec) / 1e6;
            printf("Tiempo de ejecución es de: %f segundos\n", tiempo_concurrente);
            sumaTiempo += tiempo_concurrente;
        }
        printf("El tiempo promedio de ejecución es de: %f segundos\n", sumaTiempo/5);
    }
